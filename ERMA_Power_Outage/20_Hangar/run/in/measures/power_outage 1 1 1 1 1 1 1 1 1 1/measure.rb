...
class PowerOutage < OpenStudio::Measure::ModelMeasure
  ...
  def arguments(model)
    ...
    arg_vector = OpenStudio::StringVector.new
    arg_vector << 'Power'
    arg_vector << 'Heating'
    arg_vector << 'Power and Heating'
    arg = OpenStudio::Measure::OSArgument.makeChoiceArgument('outage_type', arg_vector, true)
    arg.setDisplayName('Type of Outage')
    # arg.setDescription('The primary source of cooling used by HVAC systems in the model.')
    arg.setDefaultValue('Power')
    args << arg
  ...
  def run(model, runner, user_arguments)
    ...
    outage_type = runner.getStringArgumentValue('outage_type', user_arguments)
    ...
    if (outage_type == 'Power') || (outage_type == 'Power and Heating')
    ...
    if (outage_type == 'Heating') || (outage_type == 'Power and Heating')
      ...
      model.getThermalZones.each do |thermal_zone|
        ...
        equipments = HVAC.existing_heating_equipment(model, runner, thermal_zone)
        equipments.each do |equipment|
          # next unless equipment.name.get.include?('Hydronic')
