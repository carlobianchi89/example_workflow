...
# start the measure
class SetPowerAndHeatOff < OpenStudio::Measure::ModelMeasure
  # define the name that a user will see, this method may be deprecated as
  # the display name in PAT comes from the name field in measure.xml
  def name
    return "Set Power and Heat off"
  end
  def description
  ...
  def arguments(model)
    ...
    arg.setUnits('hours')
    arg.setDescription('Duration of the power outage in hours. Be mindfu of the 1h shift because of DST')
    ...
    # Make argument for outage type
    outage_type = OpenStudio::StringVector.new
    outage_type << 'Power and Heat'
    outage_type << 'Heat only'
    arg = OpenStudio::Measure::OSArgument.makeChoiceArgument('outage_type', outage_type, true)
    arg.setDisplayName('Outage Type')
    arg.setDescription('Select outage type for Kodiak')
    arg.setDefaultValue('Power and Heat')
    args << arg
  ...
  def run(model, runner, user_arguments)
    ...
    otg_type = runner.getStringArgumentValue('outage_type', user_arguments)
    ...
    otg_start_date_day = get_day_num_from_month_day(assumed_year, otg_period_start.month, otg_period_start.day)
    otg_end_date_day = get_day_num_from_month_day(assumed_year, otg_period_end.month, otg_period_end.day)

    # puts '++++++++++'
    # puts 'DAYS CALCULATION !!!!!!!!!'
    # puts 'Jan1'
    # puts OpenStudio::Date.new(OpenStudio::MonthOfYear.new(otg_period_start.month), otg_period_start.day, otg_period_start.year)
    # puts otg_start_date_day
    # puts otg_end_date_day
    # puts "$$$$"
    # puts otg_period_start
    # puts otg_period_end
    # puts '$$$'
    # puts otg_period_start.day
    # puts otg_period_end.day

    # puts 'FIX'
    # puts (Constants.NumDaysInMonths(2007)).class
    ...
    # puts '++++++++++'
    # puts 'ALTERNATIVE DAYS'

    # day_s = get_day_num_from_month_day(assumed_year, otg_period_start.month, otg_period_start.day)
    # day_e = get_day_num_from_month_day(assumed_year, otg_period_end.month, otg_period_end.day)
    ...
    if otg_type == 'Power and Heat'
      model.getScheduleRulesets.each do |schedule_ruleset|

        puts schedule_ruleset.name
        next if schedule_ruleset.name.to_s.include?('Setp') || schedule_ruleset.name.to_s.include?('Setpoint')
        ...
        if otg_period_num_days <= 1 # occurs within one calendar day
          ...
          (otg_start_date_day..otg_end_date_day).each do |day|
            # puts 'otg_start_date_day=' + otg_start_date_day.to_s
            # puts 'otg_end_date_day=' + otg_end_date_day.to_s
            # puts 'day=' + day.to_s
            # puts 'otg_day=' + otg_day.to_s
            # puts assumed_year
            # puts '***********'
    ...
    if (otg_start_date_day >= dst_start_day) && (otg_start_date_day <= dst_end_day)
      otg_start_hour_of_year += 1
    #   otg_start_date_day
    end
    if (otg_end_hour_of_year >= dst_start_day) && (otg_end_hour_of_year <= dst_end_day)
      otg_end_hour_of_year += 1
    #   otg_start_date_day
    end
    puts otg_start_hour_of_year
    (otg_start_hour_of_year...otg_end_hour_of_year).each do |hour|
      # otg_availability[hour-24] = 0
      # puts hour-24
    ...
    # otg_availability_schedule.setName('Outage Availability Schedule AAAAAAA')

    # puts otg_start_hour_of_year
    puts '***************'
    puts calculate_date_from_hours(otg_start_hour_of_year)
    puts calculate_date_from_hours(otg_end_hour_of_year)
    puts '***************'


    #A few base loads for Kodial have been set as constant electric consumptions, using 'Always On Discrete'
    if otg_type == 'Power and Heat'
      model.getElectricEquipments.sort.each do |item|
        schedule_name = item.schedule.get.name
        puts item.schedule.get.name
        if schedule_name.to_s.include? 'Always On'
          item.setSchedule(otg_availability_schedule)
        end
        # puts item.schedule.get.name
        puts item.schedule.get.name
        puts '***********'
    ...
    model.getThermalZones.each do |thermal_zone|
      ...
      equipments.each do |equipment|
        next if (otg_type == 'Heat only') & (equipment.name.to_s.include? 'Electric')
        # puts equipmentsunit_system.heatingCoil.get
        if (otg_type == 'Heat only') & (equipment.name.to_s.include? 'Unitary')
          next if equipment.heatingCoil.get.name.to_s.include? 'Electric'
        end
        puts equipment
    ...
    # # set outage availability schedule on all unitary objects
    # model.getAirLoopHVACUnitarySystems.each do |unit_system|
    #   unit_system.setAvailabilitySchedule(otg_availability_schedule)
    #   unit_system.supplyFan.get.to_FanOnOff.get.setAvailabilitySchedule(otg_availability_schedule)
    #   #AAAAAAAAAAHHHHHHHH Make sure this is the right coil type
    #   unit_system.heatingCoil.get.to_CoilHeatingWater.get.setAvailabilitySchedule(otg_availability_schedule)
    #   #AAAAAAAAAAHHHHHHHH Make sure this is the right coil type
    #   unit_system.coolingCoil.get.to_CoilCoolingDXSingleSpeed.get.setAvailabilitySchedule(otg_availability_schedule)
    #   unit_system.supplementalHeatingCoil.get.to_CoilHeatingElectric.get.setAvailabilitySchedule(otg_availability_schedule)
    #   # puts unit_system.name
    #   runner.registerInfo("Modified the availability schedule for '#{unit_system.name}'.")
    # end


    # model.getFanVariableVolumes.sort.each do |fan|
    #   fan.setAvailabilitySchedule(otg_availability_schedule)
    #   # puts fan.availabilitySchedule
    # end
  ...
  def calculate_date_from_hours(hour_number)
    ...
    months_list.each do |month_days|
      month_counter += 1
      month_hours = month_days*24
      total_hours += month_hours
      # puts total_hours
      if total_hours > hour_number
        # puts 'Here'
        selected_month = month_counter
        # puts selected_month
        selected_day = ((hour_number-previous_month_hours)/24).floor
        # puts previous_month_hours
        # puts otg_start_hour_of_year
        # puts selected_day
        selected_hour = hour_number - (selected_day*24+previous_month_hours)
        # puts selected_day*24+previous_month_hours
        # puts otg_start_hour_of_year
        # puts selected_hour
        break
      end
      previous_month_hours += month_hours
    end
    date = selected_month.to_s + '/' + selected_day.to_s + ' ' + selected_hour.to_s + ':00'
    return date
  end

  # def calculate_day_from_date(hour_number)
  #   months_list = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
  #   month_counter = 0
  #   selected_month = 0
  #   selected_day = 0
  ...
  def get_day_num_from_month_day(year, month, day)
...
SetPowerAndHeatOff.new.registerWithApplication
